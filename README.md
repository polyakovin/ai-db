...
### Single Source of Truth для инструментов

Каждый инструмент/платформа/провайдер получает **отдельную страницу** в `tools/`. Все ссылки на него из patterns, sources и других tools ведут **только на эту страницу** — никакого дублирования фактов в разных заметках.

Страница инструмента содержит факты о нём (модели, цены, Open Source статус, агентные возможности), а не архитектурные решения или workflow — они живут в `patterns/`.

Подробный шаблон и список категорий — в [[sources/OVERVIEW.md#правило-инструменты-и-платформы--отдельные-страницы|sources/OVERVIEW → Правило инструменты]].
