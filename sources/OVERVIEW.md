...
### Правило: инструменты и платформы — отдельные страницы

Для каждого инструмента, платформы или провайдера, на который ссылаются из patterns, создаётся отдельная страница в `tools/` (или в подпапке `tools/models/`, `tools/platforms/`).

**Какие инструменты должны иметь свою страницу:**
- API-провайдеры (OpenAI, Anthropic, Google, Mistral, DeepSeek и т.д.)
- Агентные CLI-инструменты (Claude Code, Codex CLI, Gemini CLI, MiMo Code)
- Фреймворки (LangGraph, AutoGen, CrewAI, Semantic Kernel, LlamaIndex)
- Low-code платформы (Dify, Flowise, LangFlow)
- Observability (LangSmith, Phoenix)
- Векторные БД, реранкеры и другие инфраструктурные инструменты
